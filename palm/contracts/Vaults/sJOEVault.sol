...
//import {IPalmVaultToken} from "./interfaces/IPalmVaultToken.sol";
...
contract Router is OwnableUpgradeable {
    ...
    function swapPalmVaultToken(
        ...
        if (_tokenIn == _receiptToken) {
            ...
            return IPalmVaultToken(_receiptToken).redeem(_amount);
        } else {
            // Swap _tokenIn for _receiptToken, aka deposit() that amount.
            return IPalmVaultToken(_receiptToken).deposit(_amount);
        }
    }
...
contract Vault is ERC20Upgradeable, Router, ReentrancyGuardUpgradeable {
    ...
    address[] public rewardTokens; //List of reward tokens send to this vault. address(1) indicates raw ETH
    ...
    // For use in the PALM borrowing protocol, depositFor assumes approval of the underlying token to the router,
            ...
            if (rewardTokens[i] != address(0)) {
                ...
                if (rewardTokens[i] == address(1)) {
                    ...
                    if (nativeBalance > MIN_SWAP) {
                        WETH.deposit{value: nativeBalance}();
                        swap(
                            address(WETH),
                            _underlyingAddress,
                            nativeBalance,
