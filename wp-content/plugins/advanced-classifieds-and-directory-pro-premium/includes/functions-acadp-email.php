...
function acadp_email_listing_owner_listing_approved( $post_id ) {
	...
	if ( isset( $email_settings['notify_users'] ) && in_array( 'listing_published', $email_settings['notify_users'] ) ) {	
		...
		//Mel: 03/01/22. To include IPFS URL in the email
		$ipfs_cid = get_post_meta($post_id, 'ipfs_cid', true);
		...
		$placeholders = array(
			...
			'{today}'         => date_i18n( $date_format, $current_time ),
			'{now}'           => date_i18n( $date_format . ' ' . $time_format, $current_time ),
			'{ipfs_url}'	  => sprintf( '<a href="%s">%s</a>', esc_url( 'https://ipfs.io/ipfs', 'advanced-classifieds-and-directory-pro' ) . '/' . $ipfs_cid, esc_html__( 'link', 'advanced-classifieds-and-directory-pro' ) )//Mel: 03/01/22
		);
...
function acadp_email_get_order_details( $order_id ) {
	...
	//Mel: 02/01/21. To check for crypto symbol if any
	$currency = get_post_meta( $order_id, 'currency_code', true );
        ...
        <?php foreach ( $order_details as $order_detail ) : ?>
       		<tr>
       			...
       			<td style="border-bottom:1px solid #CCC;">
					<?php
						//Mel: 02/01/22. To get the total amount instead of the price of the single item
						$amount = get_post_meta( $order_id, 'amount', true );
						echo acadp_format_payment_amount( $amount );
						//echo acadp_format_payment_amount( $order_detail['price'] );
					?>
                </td>
       		</tr>
        <?php endforeach; ?>
